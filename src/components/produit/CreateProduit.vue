...
const codeProduitConcession = ref('')
const observationsProduit = ref('')
const dureeMinimaleFacturation = ref('')
const hasSpecificiteFacturation = ref('')
const surfaceMinSpecificiteFact = ref('')
const tauxApplicableSpecificiteFact = ref('')
const iDDispositifPub = ref('')
const loading = ref(false)
const buttonDisabled = ref(false)

// define a function to create a new produit
const createProduit = () => {
    // TODO: perform some action to create a new produit
    if (!codeProduitConcession.value || !iDDispositifPub.value || !dureeMinimaleFacturation.value) {
        return
    }
    if(!iDDispositifPub.value){
        alert.value.showAlert("error", "Veuillez choisir un dispositif", "error!!")
        return
    }
    loading.value = true
    ...
    const newProduit = {
        CodeProduitConcession: codeProduitConcession.value,
        ObservationsProduit: observationsProduit.value,
        DureeMinimaleFacturation: dureeMinimaleFacturation.value,
        HasSpecificiteFacturation: hasSpecificiteFacturation.value,
        SurfaceMinSpecificiteFact: surfaceMinSpecificiteFact.value,
        TauxApplicableSpecificiteFact: tauxApplicableSpecificiteFact.value,
        IDDispositifPub: iDDispositifPub.value
    }

    // post request to the server
    axios.post('produitConcession', newProduit)
        .then((response) => {
            console.log(response)
            // notify the parent component
            props.onProduitCreated()
        })
        .catch((error) => {
            console.log(error)
    ...
    <Modal persistent v-if="showModal" @close="onCancel">
        ...
        <template #body>
            ...
            <div class="px-6 py-6 lg:px-8">
                <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Creer un Produit</h3>
                ...
                <form class="space-y-6" @submit.prevent="createProduit">
                    <div>
                        <label for="codeProduitConcession" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Code Produit</label>
                        <input type="text" v-model="codeProduitConcession" name="codeProduitConcession" id="codeProduitConcession"
                            placeholder="Code Produit"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                            required>
                    </div>
                    <div>
                        <label for="observationsProduit"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Observations</label>
                        <input type="text" v-model="observationsProduit" name="observationsProduit" id="observationsProduit"
                            placeholder="observationsProduit"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                            required>
                    </div>
                    <div>
                        <label for="dureeMinimaleFacturation"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Durée</label>
                        <input type="number" v-model="dureeMinimaleFacturation" name="dureeMinimaleFacturation"
                            id="dureeMinimaleFacturation" placeholder="Durée minimale"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                            required>
                    </div>
                    <div>
